...
routeAuth.post("/register", body(["username", "email", "password", "passwordConfirm"]).exists(), body("password").custom(validPass), denyIfLoggedIn, register);
routeAuth.put("/user", body(["username", "password", "newPassword", "newPasswordConfirm", "email"]).exists(), body("newPassword").custom(validPass), loginChecker, editUser);
